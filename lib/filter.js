...
module.exports = function(data) {
  ...
  const filterExternal = (data) => {
    return data.replace(/<a.*?(href=['"](.*?)['"]).*?>/gi, (str, hrefStr, href) => {
      ...
      if (config.nofollow.goto.enable) {
        var newHrefStr = hrefStr.replace(href, config.nofollow.goto.prefix + Base64.encodeURI(href));
        return str.replace(hrefStr, `${newHrefStr} rel="nofollow"`);
      }
